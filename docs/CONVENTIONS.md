...
- **CitationsScreen:** CRUD for citations DB. Author auto-formatted to initials + family name on blur ("יוסי ישראלי" → "י. ישראלי"). Live preview shown during typing. Single word (family name only) left unformatted. Has a guest-link section (non-selection mode only) with "העתק קישור" (clipboard copy, 2-second "הועתק!" confirmation) and "שתף בוואטסאפ" (opens `wa.me/?text=...` in new tab). Guest link URL is `{window.location.origin}/guest/{username}` — read via `getUsername()`. A "מרכז שיתוף" button in the guest-link section header navigates to `/sharing-center`. Edit/delete per citation is gated by `canEditDelete = !citation.createdByUsername || citation.createdByUsername === currentUsername` — non-owners see the citation but cannot edit it. New citations are saved with `createdByUsername: getUsername() ?? undefined`.
- **SharingCenterScreen:** Route `/sharing-center`. Accessible only from CitationsScreen (non-selection mode). On mount calls `loadSharingCenterUpdates()` to check accept/rejection notifications and refresh member list. Shows: pending invitation (accept/decline), current group members + outgoing invitation status + invite input, or "not in a group" state with invite input. "עזוב קבוצה" triggers `leaveGroup()` via `ConfirmDialog` then navigates to `/citations`. All sharing group management lives here — no share UI in App.tsx.
...
- `loadSharingCenterUpdates()` in `citationShare.ts` is the only function that checks for accept/rejection notifications and refreshes the member list. It is called on mount of `SharingCenterScreen` only — never on app startup or via visibility/focus events.
