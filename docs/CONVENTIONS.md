...
- `loadSharingCenterUpdates()` in `citationShare.ts` checks accept/rejection notifications, fetches `share:groupInvitation` from KV (delivery + cancellation detection), and refreshes the member list. It is called on mount of `SharingCenterScreen` only — never on app startup or via visibility/focus events. Returns `{ acceptedBy?, rejectedBy?, freshMembers?, invitationCancelled? }`. When `invitationCancelled` is true, the local invitation has already been cleared before the function returns.
