...
describe('One time-based + one headcount station', () => {
  it('creates schedule with mixed station types', async () => {
    const user = userEvent.setup()
    upsertGroup(makeGroup4Members())
    renderApp()

    // Step 1: 2 stations, make station 2 headcount
    await user.click(screen.getByRole('button', { name: '2' }))

    // The second "כוח אדם" button corresponds to station 2
    const headcountButtons = screen.getAllByText('כוח אדם')
    await user.click(headcountButtons[1])
    ...
    // Step 3: time-based station gets participants; headcount station needs checkbox selection
    // For simplicity, just select 1 participant for headcount and proceed
    ...
    await waitFor(() => {
      const schedule = getSchedules()[0]
      expect(schedule.stations).toHaveLength(2)
      const timeStation = schedule.stations.find(s => s.stationType === 'time-based')
      const headStation = schedule.stations.find(s => s.stationType === 'headcount')
      expect(timeStation).toBeTruthy()
      expect(headStation).toBeTruthy()
    })
  })
})
