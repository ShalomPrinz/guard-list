...
describe('generateShortListSchedule', () => {
  ...
  it('generates schedule with correct name and date', () => {
    ...
    const result = generateShortListSchedule(group.id, mockStations, 14, 60, 4, mockStorage)
    expect(result).not.toBeNull()
    expect(result!.name).toBe('רשימה קצרה')
    expect(result!.groupId).toBe(group.id)
    expect(result!.unevenDistributionMode).toBe('equal-duration')
    expect(result!.date).toMatch(/^\d{4}-\d{2}-\d{2}$/)
  })

  it('distributes warriors round-robin across stations', () => {
    const group = createMockGroup(10, 8)
    vi.mocked(mockStorage.getItem).mockReturnValueOnce(JSON.stringify([group]))
    ...
    expect(Math.abs(st1Participants - st2Participants)).toBeLessThanOrEqual(1)
  })
  ...
  it('assigns correct duration to each participant', () => {
    ...
    const result = generateShortListSchedule(group.id, mockStations, 14, minutesPerWarrior, 4, mockStorage)
    expect(result).not.toBeNull()
  ...
  it('handles midnight crossover correctly', () => {
    ...
    // Last (4th) participant: should cross into next day (02:00 to 03:00)
  ...
  it('handles exactly matching warrior count to base members', () => {
    const group = createMockGroup(8, 5)
    vi.mocked(mockStorage.getItem).mockReturnValueOnce(JSON.stringify([group]))
  ...
  it('generates deterministic output when seeded', () => {
    ...
    const result1 = generateShortListSchedule(group.id, mockStations, 14, 60, 6, mockStorage)
  ...
  it('many stations with few warriors distributes evenly', () => {
    ...
    const result = generateShortListSchedule(group.id, manyStations, 14, 60, 3, mockStorage)
    expect(result).not.toBeNull()
